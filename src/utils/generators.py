...
class AudioFixedChunksGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, *, sequence_max_length: float, window_length: float, load_mode: str = 'path',
        ...
        if load_mode == 'path':
            ...
            if isinstance(load_path, str):
                ...
                raise AttributeError(
                    'load_path must be a string path to the directory with data and label files. Got %s' % (load_path))
            ...
            if isinstance(data, dict):
                ...
                raise AttributeError(
                    'With \'data\' load mode the data should be in dict format. Got %s.' % (type(data)))
    ...
    def _preprocess_all_data(self):
        for key, value in self.data.items():
            ...
            cut_sequence = self._preprocess_cut_audio(cut_sequence, sample_rate, self.data_preprocessing_mode,
                                                      self.num_mfcc)
